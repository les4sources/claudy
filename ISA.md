...
updated: 2026-09-07T19:20:00+02:00
---
...
- [ ] ISC-5: La page calendrier (`pages#calendar`) affiche les occupations **regroupées et colorées par séjour** (`stay_id`) — hébergement (réservations de chambres), espaces, camping, van — avec navigation mois/semaine (`?date=`) et **modale de composition** au clic (`stays#show`). Les `Booking` legacy sans séjour retombent sur le popover d'info historique. (révisé epic #66, 2026-07-18)
...
- name: Calendar
  description: |
    Root UI (pages#calendar) — simple_calendar gem with custom overrides in app/calendars/simple_calendar/.
    **epic #66 (2026-07-18)** : les occupations sont **regroupées et colorées par séjour** (`stay_id`, via `has_one :stay, through: :stay_item`) — hébergement (réservations de chambres), espaces, camping, van — chaque bloc cliquable ouvre une **modale de composition** (`stays#show`, Turbo Frame + Stimulus `stay-details`). Les `Booking` legacy sans séjour retombent sur le popover d'info historique. Navigation mois/semaine via `?date=`.
    **epic #81 Phase 9 (2026-07-19)** : `Booking` et `SpaceBooking` sont désormais des **objets d'occupation internes** — le **séjour est le point d'entrée UNIQUE de création/édition** (admin via `resources :stays`, public via le funnel `/reservation`). Les canaux de création directe (admin `bookings`/`space_bookings` new/create, public legacy `public/bookings` new/create) sont **retirés**. **Fusion de séjours** disponible depuis le calendrier (Phase 2, `Stays::MergeService`). L'édition legacy `bookings/:id/edit` reste un **fallback orphelin** tant que le **backfill des orphelins n'a pas tourné en prod** (cf. Decision 2026-07-19).
...
- name: StayComposite
  description: |
    ...
    **État de build (2026-07-20)** — **TOUT EST MERGÉ ET DÉPLOYÉ.** Epics #55, #66, #81 (« séjour = point d'entrée unique », 9 phases : fusion `Stays::MergeService`+mode fusion calendrier, prix imposé `price_override_cents`, `booking_type` colonne, chambres seules, `DraftReconstructor` partagé, duplication, retrait création directe) sur main ; #94 (vetos en nuits, dos-à-dos OK) + #99 (édition legacy retirée, `Stays::DestroyService`) clos ; backfill prod 0 orphelin ; funnel public réparé de bout en bout (paiement Stripe #104, page Checkout détaillée #105, flux 2 emails #106, devis en drawer #107) ; calendrier un-bloc-par-séjour #108 ; espaces composés dates/durées #109 + migration de données. Déploiement Hatchbox fiabilisé (drop-in systemd, un seul restart). Suite : **826 ex / 0 failure**. **Prochaine exploitation : assainissement de l'historique 2023→ via la fusion** (l'outil a été construit pour ça). Cf. `SEQUENCING.md`.
    **État de build (2026-07-18)** — **epics #55 (activités) et #66 (séjour composable) livrés** (PR empilées non mergées) :
      - epic #55 : activités intégrées au séjour (`Stay has_many :experience_bookings`), validation porteur double-canal, système acompte/solde global sur `/sejour/:token`. PR #56→#61.
      - epic #66 : **CRUD `Stay` admin composable** (`StaysController` new/create/edit/update/destroy) = **point d'entrée unique** de création côté admin, réutilisant `Reservations::Builder` **sans Stripe** (mode admin : force-dispo, statut `pending`/`confirmed` au choix, client existant/nouveau) ; **espaces, camping (capacité globale), van, repas datés** persistés (`SpaceBooking`, `CampingBooking`/`VanBooking` via `StayItem` polymorphe, `MealOrder` en `has_many` direct) ; **calendrier regroupé/coloré par séjour + modale de composition** ; **le Builder crée désormais les `Reservation` de chambres** (Phase 6, correctif) — sans quoi les séjours Stay-first étaient invisibles au calendrier et ne posaient pas le veto de dispo. PR #67→#72, suite **584 ex / 0 failure**, vérif navigateur end-to-end OK. ⚠️ **Renverse partiellement la clarification #26** : l'occupation calendrier ne tient PAS par `lodging_id` + dates seuls — elle exige les `Reservation` de chambres (source de vérité de `Lodging#available_between?`).
    **À caler avant prod (epic #66)** : capacités globales camping/van par défaut (30 pers / 5 véhicules) ; mapping clés de prix espaces (`grande_salle`…) ↔ `Space` nommées ; devis « live » (rendu au submit, pas temps-réel).
  satisfies: []
...
- name: Activities
  description: |
    Activités payantes portées par un membre du collectif (tarif horaire unique — règle P5) ou un prestataire externe (tarif négocié). Modèles déjà en place — `Experience` (price_cents + min/max_participants + duration) et `Service` (price_cents + human). Réservables dans le cadre d'un séjour OU en standalone.
    **État de build (2026-07-16)** — **intégration au séjour-composite LIVRÉE** via epic #55 (6 phases, une PR empilée par phase, non encore mergées : #56→#61 ; Forge indisponible → implémentées par Engineer/Claude ; suite 501 ex / 0 failure ; passe navigateur fonctionnelle OK) :
      - Phase 1 (#56) — `ExperienceBooking` dans `Stay#total_amount_cents`, service de barème unique `Pricing::ExperienceLine`, sémantique acompte/solde, acompte hors activités.
      - Phase 2 (#57) — validation porteur double canal (admin + lien email `signed_id`), statut `refused` + `refusal_reason`, scoping porteur/admin.
      - Phase 3 (#58) — paiement du **solde exigible** sur `/sejour/:token` (Stripe, réutilise l'acompte) ; activités `pending` exclues de l'exigible, `confirmed` incluses.
      - Phase 4 (#59) — étape **Activités** dans le funnel `/reservation` (4 étapes), créneaux datés, `Builder` persiste les EB `pending`.
      - Phase 5 (#60) — relance J-14 du solde (idempotente, sans blocage) ; rail email de sélection confirmé cohérent (EB `pending`).
      - Phase 6 (#61) — CRUD admin d'activité sur un séjour (ajout avec choix `pending`/`confirmed`, édition, retrait), impact total.
    **Reste hors périmètre de l'epic #55** (garde le statut `partial`) : attachement à un **Pôle** et distinction **tarif membre-collectif vs externe** (P5) ; rendre `Service` réservable.
  satisfies: []
  depends_on: [Poles]
  parallelizable: true
  status: partial
...
- 2026-09-07 — **Volet E « Administratif & Finances » cadré en 10 issues (#239→#248) depuis la note vocale de Michael « 4S : Admin dans Claudy ».** Les lots A et B du chantier Finances étant livrés, ce volet couvre les lots C et E du plan du 27/07 : pôles pilotables (config + page + finances), factures d'achat avec validation par le pôle et file « À payer », registre des ventes OkiOki, notes de frais et de mission, commentaires polymorphes + notifications, caisse, rémunération des porteurs et des organisateurs d'événements, batch cooking, tiny house 50/50, dépôt-vente. Décisions structurantes : (1) l'écriture d'achat se génère à `to_pay`, le paiement est un rapprochement sur `440000` avec `document` — une dépense couverte par une facture ne s'affecte plus directement sur la charge ; (2) le registre des ventes **ne génère aucune écriture** (règle B2 anti-double-compte conservée jusqu'à la bascule Winbooks) ; (3) une seule caisse comptable (plan C1), confirmée par Michael le soir même ; (4) tous les payables (factures, notes, relevés porteurs, parts d'événements, tiny house, dépôt-vente) convergent vers un concern `Payable` et l'écran « À payer » ; (5) OkiOki n'a pas d'API — l'émission de factures reste manuelle ; Michael a sollicité OkiOki pour une API/MCP et **reporte la décision Billit à fin T4 2026** ; (6) pas de scheduler dans l'app : rappels et demandes = rakes dry-run + actions explicites ; (7) batch cooking : **un compte sourcier personnel par cuisinier** (enfants compris), soldé par virement — pas de compensation sur le ménage ; (8) notes de frais : la compta encode seule, le self-service est reporté ; tarif kilométrique 0,47 €/km, daté. **Découverte du soir (#250)** : le compte Stripe de Tranches de Vie fait des versements manuels, que l'API Stripe refuse de décomposer — la conjecture « un versement se lit par ses transactions » (#187) est réfutée pour ce compte ; critère désormais : un compte Stripe peut être tenu en grand livre (mode `ledger`), le versement y est un virement interne 580000. Constaté aussi que `VentilateStripePayout` n'était branché sur aucun écran. Détail et checklists de phases dans `SEQUENCING.md` Volet E.
- 2026-07-21 (nuit) — **Marathon de co-construction en direct : ~25 chantiers livrés en une soirée, tous mergés et vérifiés (suite finale 966 ex/0, passe navigateur 9/9).** Détail exhaustif dans `SEQUENCING.md` (Volet C). Décisions produit majeures actées : **camping/van persistés PAR NUIT** (plages contiguës sans nouvelle table, ventilation largest-remainder = devis exact, grilles rechargées au changement de dates via frame `stay_compose_grids`) · **terrasse 2,50 €/pers/jour** (CampingBooking kind "terrasse", admin uniquement, pas une nuitée) · **« Les 2 salles » retiré** (remise duo automatique 390/495 j · 250/335 soir · J+S +150, historique converti en paires prix conservés, espace soft-deleté) · **repas : 2 formules réelles** (« Formule complète » n'a jamais existé) · **fourre-tout par OTA** (Client Airbnb / Client Booking.com, anti-fuite inter-séjours spécifié) · **équipe** : rôles activables par membre, statut éditable, Human inactif = connexion refusée (`active_for_authentication?`), utilisateurs restreints à leur planning d'activités · **fusion depuis la fiche client** (contrôleur stay-merge généralisé scope+returnUrl, anti-open-redirect) · **notes séjour** (interne + publique ActionText, consolidation à la fusion avec provenance — revue Forge F1/F5 appliquée) · section veilleur : **les check-out du jour n'apparaissaient JAMAIS** (dernière nuitée = veille) — corrigé, bandeaux colorés par état. Epic **#126 Portail client + coworking en PACKS de journées** cadré interactivement (packs 1/5/10/20 j à 20/80/160/300 €, capacité simple 3/j lun-ven, expiration 12 mois, annulation < 8h, OTP email anti-énumération, fourre-tout exclus) → issues #127/#128 agent:ready + #129/#130 gated + #133 (modification de séjour client : demande VALIDÉE par l'équipe, delta sur solde, IBAN remboursement « dans les 10 jours qui suivent le séjour ») + #124 (Paramètres > Tarifs). **Leçons process durement acquises** : jamais de `rg` en bout de chaîne shell (masque l'exit code — 2 pushes rouges attrapés tard) ; un checkout partagé entre agents concurrents balaie les éditions non commitées (action `recent_activity` perdue puis restaurée) ; les variantes Tailwind `peer-checked:`/`after:*` ne sortent pas du JIT sous Vite → composant `.ui-switch` en CSS pur.
- 2026-07-20 (soir) — **Journée de fiabilisation post-epic-#81 : le funnel public encaisse ENFIN, 6 PRs mergées (#104→#109), déploiement fiabilisé.** (1) **Le paiement public n'a JAMAIS fonctionné depuis la création du funnel** : le form final postait via Turbo et la redirection externe vers Stripe Checkout est bloquée par CORS quand un fetch la suit → clic silencieusement mort, zéro erreur JS, zéro Sentry (le serveur répondait juste). Fix `data: { turbo: false }` sur le FORM (PR #104 — le `data: { turbo: false }` historique était posé sur le `redirect_to`, où il part en flash) + fallback échec-Stripe **stay-first** (`public_stay_path` ; `builder.booking.token` crashait en 500 sur un panier sans hébergement, APRÈS création du Stay + envoi de l'email + vidage du draft → doublons à chaque re-clic). Règle générale actée : tout form dont la réponse peut rediriger hors domaine porte `data-turbo=false`. (2) **Page Stripe lisible** (PR #105) : nom = « Séjour aux 4 Sources · du X au Y », description = acompte/solde vs total + composition, `customer_email` prérempli — `PayService` couvre acompte ET solde. (3) **Décision produit (Michael) : l'acompte confirme la réservation → flux à DEUX emails** (PR #106) — à la soumission « demande enregistrée » avec montant de l'acompte dû + lien de paiement direct (filet anti-abandon Stripe) ; au premier encaissement webhook d'un séjour `pending`, « acompte bien reçu, validation en cours ». Coquilles Slim corrigées au passage (deux `|` consécutifs se concatènent SANS espace ; `| = "…"` sort le code verbatim) avec specs verrouillant la prose rendue. (4) **Devis en drawer** (PR #107, demande UX Michael) : barre récap sticky (total TVAC permanent) + slide-over droite desktop / bottom-sheet mobile sur les 3 étapes de composition, total mis à jour par le turbo_stream du devis (`#quote_sticky_total` + frame `reservation_quote`), CTAs d'étape JAMAIS dans le drawer (le submit Stripe reste dans son form turbo:false). (5) **Calendrier : UN bloc par séjour et par jour** (PR #108, demande lisibilité Michael) : presenter `Calendar::DayStayBlocks` (agrégation d'affichage pure, keyée `stay.id`) + partial `_day_stay_block` — chambres/gîte + espaces + camping + van en badges compacts sur un bloc coloré unique ; les occupations sans séjour gardent leur rendu legacy ; le mode fusion y gagne (un ✓ par séjour). (6) **Bug espaces composés** (PR #109, déterré par la vérif navigateur de #108) : la composition persistait la clé de PRICING (`journee`/`soiree`/`journee_et_soiree`) dans `space_reservations.duration` (vocabulaire canonique : `day`/`evening`/`fullday`) → « période non précisée » partout, et `from/to_date` sur la fenêtre du séjour au lieu des dates réelles. Fix à l'ingestion (`DURATION_BY_PERIOD` + inverse dans `DraftReconstructor`, sinon devis d'édition à 0) + **migration de données** chirurgicale (durées + réalignement from/to sur les bornes des réservations vivantes, uniquement où ça diverge). (7) **Double-restart Hatchbox résolu** : claudy était la SEULE unité systemd du serveur avec `ExecReload` (SIGUSR2 = hot-reload Puma qui ne relit ni env ni gems) → drop-in `override.conf` vidant `ExecReload`, `reload-or-restart` fait désormais un vrai restart — **« Deploy on push » activable**. Suite sur main mergé : **826 ex / 0 failure**. **Leçon process** : un agent-worktree repris via SendMessage après crash API peut perdre son worktree et committer dans le repo principal (attrapé par la revue Forge : diff `main...branche` vide) — toujours vérifier le diff de branche non vide avant revue/PR.
- 2026-07-20 (après-midi) — **Epic #81 mergé + déployé + exploité ; #94 clôt le chantier séjour.** Pile #90→#101 mergée sur main (leçon : `gh pr merge --delete-branch` FERME sans recibler les PRs dont c'était la base — recibler sur main AVANT de merger), hotfix calendrier public #100 (erreur d'indentation Slim = sortie de bloc silencieuse → 500 prod), backfill prod : **0 orphelin** (817 bookings déjà rattachés, 40 SpaceBookings créés). **#94** : les vetos de dispo raisonnent en NUITS `[arrivée, départ)` — `last_night = [to-1, from].max` (le `max` préserve `available_on?(d,d)` ; une borne exclusive naïve donnait une plage vide = tout dispo = surbooking silencieux) — les rotations dos-à-dos passent, les vrais chevauchements restent refusés ; les `Unavailability` restent des journées pleines inclusives (délibéré). PR #102 (#99) et #103 (#94) mergées + déployées.
- 2026-07-20 — **Issue #99 : retrait FINAL des écrans d'édition legacy Booking/SpaceBooking + suppression d'un séjour qui libère ses occupations.** Pré-condition satisfaite (le backfill des orphelins a tourné en prod : invariant 0 orphelin confirmé — 817 bookings, 579 space_bookings tous rattachés), le **fallback orphelin assumé** de la Decision 2026-07-19 est donc **levé**. `bookings#edit` / `space_bookings#edit` deviennent de **PURES redirections** : un `Stay` vivant → `edit_stay_path` ; cas résiduel sans séjour vivant (séjour soft-deleté à la main) → **fiche `#show` + alerte** invitant à relancer `rake stays:backfill_missing`. **Plus jamais de rendu du form legacy.** **`#update` retirés** (actions + routes : `resources :bookings|:space_bookings, except: [..., :update]`) — preuve `rg` : l'unique soumission PATCH vers `booking_path`/`space_booking_path` était le form legacy lui-même. **Vues supprimées** (rendues uniquement par le flux edit, aucun autre appelant — preuve `rg render`) : `bookings|space_bookings/edit.html.slim`, `_form.html.slim`, `form/{info,type,reservation,payment,options,notes}` ; les partials partagés avec `show` (`_popover`, `_status_callout`, `_table`, `show/*`) sont **conservés**. **Services morts supprimés** (plus aucun appelant après retrait de `#update`) : `Bookings::UpdateService`, `SpaceBookings::UpdateService` (+ spec) — **corrige** la Decision 2026-07-19 qui les listait « Conservés ». **Fallbacks simplifiés** : les branches `stay` nil (calendrier `_bookings`/`_day_bookings`, `day`, `other_*`, `_table`, `past`, `show`) ne proposent plus d'édition legacy et pointent vers la **fiche `#show`** (nil-safe pour le cas résiduel). **Suppression d'un séjour (`stays#destroy` → nouveau `Stays::DestroyService`)** : `Stay#soft_delete!` ne cascade QUE sur `stay_items` + `meal_orders` (leurs classes portent `has_soft_deletion` → `soft_delete_dependents`), **PAS sur les bookables** (association polymorphe) — supprimer un séjour laissait donc `Booking`/`SpaceBooking` + leurs `Reservation`/`SpaceReservation` **VIVANTS**, blocs fantômes qui occupaient le calendrier et posaient le veto sans séjour d'attache. Le service soft-delete explicitement chaque bookable AVANT le Stay : `Booking` cascade sur `reservations` (chambres rendues) ; `SpaceBooking` + `destroy_all` de ses `space_reservations` (qui n'ont **pas** de soft-deletion et ne cascadent pas — miroir de `AdminUpdater#reconcile_spaces!`) ; camping/van suffisent. Après suppression : **plus aucune occupation vivante au calendrier ni au veto**, **`Payment` conservés** (trace financière). **Découverte hors périmètre** : `MealOrder` cascade bien via le Stay (soft-delete), mais **`ExperienceBooking` NON** (pas de `has_soft_deletion` → absent de `soft_delete_dependents`) — un séjour supprimé laisse ses activités `ExperienceBooking` vivantes (occupation de créneaux d'activité, domaine distinct du calendrier chambres/espaces). Laissé **hors périmètre** de #99 (« ne touche pas les modèles/tables ») → **follow-up recommandé**. `MergeService` inchangé (soft-delete ses sources vidées, jamais via `DestroyService`) — spec de non-régression verte. **Ne touche PAS** : `#show`, `#destroy` des bookings/space_bookings hors wiring, chemins token publics, `#index`/`past`/`search`, modèles/tables. Specs : `unified_edit_redirect` réécrit (redirection + alerte, plus d'écran legacy), `legacy_edit_removed` ajouté (PATCH/PUT → `RoutingError`), `destroy_service_spec` ajouté, `stays_admin_crud` étendu (destroy libère l'hébergement). **Mode de livraison** : Engineer (Claude-family) en worktree isolé, commits atomiques FR, à vérifier au navigateur.
- 2026-07-19 — **Epic #81 Phase 9 (finale) : dépréciation effective de la création directe.** Aboutissement du pivot Stay-first entamé aux epics #26/#66/#55. Le **séjour** (`resources :stays`) est désormais le **point d'entrée UNIQUE de création ET d'édition côté admin**, et le **funnel natif `/reservation`** le point d'entrée UNIQUE côté public. `Booking` et `SpaceBooking` deviennent des **objets d'occupation internes** (chambres × dates, espaces × dates) — ils n'ont plus de canal de création propre, ils naissent du séjour (via `Reservations::Builder`/`Stays::EnsureFor*`) ou du funnel public. **Canaux de création directe retirés** : (1) admin — `resources :bookings`/`:space_bookings` perdent `new`/`create`, actions contrôleur `#new`/`#create` supprimées, vues `bookings/new` + `space_bookings/new` supprimées, boutons « Nouvelle réservation » des index repointés vers « Nouveau séjour » (`new_stay_path`), liens « Dupliquer » des `show` retirés (la duplication vit au niveau du séjour via `new_stay_path(duplicate_from:)`) ; (2) public legacy — `Public::BookingsController#new`/`#create` + routes retirés, vue `public/bookings/new` + partials de form supprimés, liens du calendrier public et des mailers (`booking_canceled`, `booking_request_rejected`) repointés vers le funnel natif `public_reservation_start`. **Services de création morts supprimés** (preuve `rg` exhaustive, plus aucun appelant hors specs) : `Bookings::CreateService`, `SpaceBookings::CreateService`, `Public::Bookings::CreateService`, `Bookings::DuplicateService`, `SpaceBookings::DuplicateService`. **Conservés** : `*::UpdateService` (édition), `Stays::DuplicateService`, `Reservations::Builder`/`Draft`, `Stays::EnsureForBooking`/`EnsureForSpaceBooking`, `Payments::CreateService` (garantit toujours un Stay). **Conservés côté public** : page client token `reservation/:token` (`public/bookings#show`), `espaces/:token` (`public/space_bookings#show`), `edit_estimated_arrival`/`update_estimated_arrival` (services aux clients existants). **Aucun modèle, table, chemin token ni email transactionnel supprimé** — seuls les LIENS dans les emails changent. **Fusion de séjours** disponible depuis le calendrier (epic #81 Phase 2, `Stays::MergeService`). ⚠️ **Fallback orphelin assumé (séquencement figé)** : l'édition legacy `bookings/:id/edit` + `space_bookings/:id/edit` reste servie tant que le **backfill des orphelins n'a pas tourné en prod** — un `Booking`/`SpaceBooking` sans `Stay` vivant retombe sur l'écran d'édition legacy (sinon redirige vers `edit_stay_path`). **À exécuter en prod** : le backfill qui crée un `Stay` pour tout `Booking`/`SpaceBooking` orphelin (cf. `spec/tasks/stays_backfill_missing_spec.rb`), après quoi le fallback legacy pourra être retiré (phase ultérieure). `rspec` complet **vert** après retrait (specs de création directe retirées, spec de preuve de retrait `spec/requests/direct_creation_removed_spec.rb` ajoutée). **Mode de livraison** : Engineer (Claude-family) en worktree isolé, commits atomiques, à vérifier au navigateur (passe groupée finale).
- 2026-07-18 — **Epic #66 « Séjour composable Stay-first » livré (5 phases + 1 correctif).** Cadrage interactif avec Michael puis exécution en **loop autonome** (Engineer, PR empilées non mergées, une par phase). Décisions produit : point d'entrée unique de création = le séjour (**CRUD admin**), réutilisation `Reservations::Builder` **sans Stripe** (force-dispo, statut au choix, client existant/nouveau), barème **B2C forfaitaire**, **camping = capacité globale** du terrain (pas d'emplacements nommés), périmètre **tente + van + repas datés** (pizza party → site boulangerie ; hamac → `RentalItem`), calendrier regroupé/coloré par séjour + modale. Phases : (1) CRUD admin hébergement+activités ; (2) espaces ; (3) camping/van/repas persistés (nouveaux modèles + migration) ; (4) calendrier par séjour ; (5) modale ; **(6) correctif** — le Builder ne créait pas les `Reservation` de chambres → séjours **invisibles au calendrier + veto de dispo cassé** (surbooking silencieux), trouvé en **vérif navigateur** ; corrigé pour admin **et** funnel public (décision Michael). PR **#67→#72**, `rspec` **584 ex / 0 failure**, vérif navigateur end-to-end OK (création → occupation chambres → calendrier → modale → veto, entanglement Grand-Duc vérifié). **Renverse partiellement la clarification #26** : l'occupation calendrier exige les `Reservation` de chambres, pas seulement `lodging_id` + dates. **À caler avant prod** : capacités camping/van par défaut (30/5), mapping clés prix espaces ↔ `Space`, devis live. **Mode de livraison** : Forge indisponible (quota Codex jusqu'au 19/07) → **Engineer (Claude)** ; PR non auto-mergées (review + déploiement Hatchbox manuel par Michael).
- 2026-07-16 — **Chantier « Activités intégrées au séjour-composite » livré (epic #55, 6 phases).** Suite à la priorité produit actée le 15/07. Cadrage interactif avec Michael puis exécution en **loop autonome** phase par phase. Décisions produit de Michael : (1) rattachement `Stay has_many :experience_bookings` **direct** (pas via `StayItem`) ; (2) sélection d'activités **dans le funnel** (si créneaux dispo) **et** via relance email ; (3) paiement non dans l'acompte mais dans un **solde exigible** payable à tout moment sur `/sejour/:token` **après validation porteur** — élargi à un **vrai système acompte/solde global du séjour** (pas seulement les activités) ; (4) validation porteur **double canal** (compte admin + lien email, refus motivé) ; (5) relance J-14 sans blocage ; (6) CRUD admin avec choix `pending`/`confirmed` à l'ajout. Livré en **6 PR empilées non mergées (#56→#61)**, suite 501 ex / 0 failure, passe navigateur fonctionnelle OK (résidu : rendu pixel — daemon screenshot cassé — et devis-live à spot-checker). **Mode de livraison** : Forge indisponible (quota Codex épuisé jusqu'au 19/07 + `gpt-5.4` refusé sur compte ChatGPT) → implémenté par **Engineer (Claude-family)** ; PR non auto-mergées (review Michael avant merge, déploiement Hatchbox manuel). Voir Feature `Activities` (état de build 2026-07-16). **Reste hors périmètre** : attachement Pôle + tarif membre/externe (P5), `Service` réservable.
- 2026-07-16 — **Décision produit (Michael) : basculer le calendrier sur `Stay` + créer un CRUD `Stay`** (formulaire de composition de séjour côté admin). ⚠️ **Renverse la clarification architecturale de l'epic #26** (« `Booking` = objet d'occupation calendrier, `Stay` = paiement + page publique »). Cible : l'utilisateur compose un séjour complet depuis un formulaire `Stay`, et le calendrier se lit par séjour. **Préalable posé** : l'intégration des activités (epic #55, ci-dessus) — faite avant ce chantier. Le chantier calendrier-Stay + CRUD Stay reste **à cadrer** (changement d'archi, pas à improviser) ; `ISA.md` (Features `StayComposite`/`BookingFlow`, ISC-5 « calendrier affiche bookings + space_bookings ») devra être révisé quand il sera lancé.
...
- 2026-07-20 — C/R/L : un vocabulaire par colonne, mappé aux frontières :
  - **conjectured** : (epic #66, Phase 2) la composition d'espaces pouvait persister telles quelles les clés de PRICING (`journee`/`soiree`/`journee_et_soiree`) dans `space_reservations.duration`, et poser `from/to_date` du SpaceBooking sur la fenêtre du séjour « par cohérence avec `recompute_aggregates!` ».
  - **refuted by** : la vérification navigateur du calendrier unifié (2026-07-20) — TOUTES les salles composées s'affichaient « période non précisée » (le vocabulaire d'affichage tranche 1 est `day`/`evening`/`fullday`) et « du 21 au 23 » pour une salle du seul 22 ; le devis d'édition aurait silencieusement perdu son tarif si on avait corrigé l'affichage sans mapper l'inverse dans `DraftReconstructor`.
  - **learned** : une colonne = UN vocabulaire canonique, converti aux frontières (ingestion ET reconstruction), jamais deux vocabulaires cohabitants ; et les dates d'un objet d'occupation sont ses dates RÉELLES, pas celles de son agrégat parent — l'agrégat se dérive, jamais l'inverse. Les données historiques divergentes se réparent par migration chirurgicale (seulement où ça diverge des réservations vivantes).
  - **criterion now** : specs `builder_spaces_spec` (« dates réelles + vocabulaire canonique ») : durée persistée ∈ vocabulaire canonique + décorateur lisible, from/to = min/max des réservations, aller-retour édition qui retrouve son tarif.
