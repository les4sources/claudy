...
- [ ] ISC-14: Anti — `POST /api/v1/bookings` (ou toute écriture API v1) → `404`/`405` (read-only en v1).
...
- name: ApiV1
  description: Read-only JSON API under namespace :api { :v1 } — Bearer token auth via AGENT_API_TOKEN, jbuilder serialization, OpenAPI spec at /api/v1/openapi.json, excludes soft-deleted and Stripe identifiers. Built per Plans/l-app-est-actuellement-accessible-woolly-cocoa.md, merged in commit e832bd6.
  satisfies: [ISC-6, ISC-7, ISC-8, ISC-9, ISC-13, ISC-14]
...
<!-- TODO author Changelog (C/R/L entries) as conjectures get refuted and learnings accumulate. -->
