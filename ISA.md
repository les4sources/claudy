...
- name: StayComposite
  description: |
    ...
    **État de build (2026-07-15)** — n'est plus `horizon` : le modèle `Stay` est **livré en prod** (tranche 1, 2026-05-30 : Customers + StayComposite + LegacyBookingMigration). En cours : **epic #26 « Payment devient Stay-first »**, 4 phases (une PR par phase) :
      ...
      - Phase 3 — canal admin/OTA sur Stay (chaque Booking → Stay auto + Customer upserté par email, rake one-shot idempotente, Payment admin porte `stay_id`) — **à faire**.
      - Phase 4 — verrouillage (`payments:verify_stay_links` → 100 %, Payment sans `stay_id` invalide, `payments.booking_id` nullable en DB, zéro Payment perdu / PaperTrail intact) — **à faire**.
