module CalendarHelper
  def button_to_next_month(current_date, data = {}, options = {})
    link_to(params.permit(:date, :no_title).merge(date: current_date.next_month), class: "btn-page-header-with-icon", data: data) do
  ...
  def buttons_to_next_months(current_date, data = {})
    content_tag(:div, class: "inline-flex rounded-md shadow-sm", role: "group") do
      links = []
      ...
      links << link_to(params.permit(:date).merge(date: current_date + 2.months), class: "hidden md:inline-flex btn-group-page-header-with-icon bg-blue-200", data: data) do
        l(current_date + 2.months, format: "%B %Y")
      end
      links << link_to(params.permit(:date).merge(date: current_date + 3.months), class: "hidden md:inline-flex btn-group-page-header-with-icon border rounded-r-md bg-blue-300", data: data) do
  ...
  def button_to_previous_month(current_date, data = {}, options = {})
    link_to(params.permit(:date, :no_title).merge(date: current_date.prev_month), class: "btn-page-header-with-icon", data: data) do
      if options[:no_label].nil?
  ...
  def buttons_to_previous_months(current_date, data = {})
    content_tag(:div, class: "inline-flex rounded-md shadow-sm mr-2", role: "group") do
      links = []
      links << link_to(params.permit(:date).merge(date: current_date - 3.months), class: "hidden md:inline-flex btn-group-page-header-with-icon border rounded-l-lg bg-blue-300", data: data) do
